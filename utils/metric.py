...
class MetricEvaluator:
	...
	def _compute_clip_metrics(self, gts, preds, tqdm_stream):
		...
		for i in range(0, len(ids), batch_size):
			batch_ids = ids[i : i + batch_size]
			batch_preds = [preds[k][0] for k in batch_ids]
			batch_refs = [gts[k] for k in batch_ids]
			batch_paths = [os.path.join(Config.coco_val_image, k) for k in batch_ids]

			cand_tokens = clip.tokenize(batch_preds, truncate=True).to(device)
			
			images = []
			for p in batch_paths:
				images.append(self.preprocess(Image.open(p)))
			...
			with torch.no_grad():
				...
				# CLIP-S (Image-Text): Scale by 2.5
				...
				for j, refs in enumerate(batch_refs):
					ref_tokens = clip.tokenize(refs, truncate=True).to(device)
					ref_feats = self.clip_model.encode_text(ref_tokens)
					ref_feats = ref_feats / ref_feats.norm(dim=-1, keepdim=True)
					
					# CLIP-S-Ref (Text-Text): No scaling
